...
SYSTEM_PROMPT = (
    ...
    "- INTERPRETACIÓN TEMPORAL DE LA METADATA: la columna `Fecha: YYYY-MM-DD` "
    "del header ES la fecha de cobertura del fragmento. Si el usuario "
    "pregunta por un mes (abril, mayo, último trimestre, etc.) y tenés un "
    "chunk con `Fecha: 2026-04-17`, ESO ES abril 2026 y debés usarlo. "
    "NUNCA respondas 'no encuentro información sobre <mes>' si tenés al "
    "menos un chunk cuya `Fecha` cae dentro de ese mes — leelo y respondé "
    "con lo que dice.\n"
    ...
    "- Antes de declarar 'no encuentro información', releé las fechas de los "
    "chunks recibidos: si la query menciona un mes y hay chunks de ese mes, "
    "respondé con esos chunks aunque no contengan literalmente la palabra "
    "del mes.\n"
    "- Si efectivamente ningún chunk cubre el período/tema pedido, decí "
    "explícitamente: 'No encuentro información sobre <tema> en los informes "
...
FOLLOWUP_PROMPT = (
    ...
    "- GROUNDING AL CORPUS: si en el contexto del usuario aparece el bloque "
    "'=== ALCANCE DEL CORPUS ===', usalo como única fuente de verdad sobre "
    "qué períodos y qué tipos de datos puede responder el bot. NO sugieras "
    "preguntas sobre meses/años fuera del rango indicado. NO sugieras "
    "comparaciones numéricas de variables que no estén listadas como "
    "disponibles vía SQL (ej. reservas, bonos, commodities, equity — esos "
    "solo se pueden responder de forma narrativa, no con cifras exactas).\n\n"
...
FILTER_EXTRACTOR_PROMPT = (
    ...
    "Recibirás una query en español, el rango temporal del corpus, una lista "
    "de archivos disponibles con fecha ISO (YYYY-MM-DD), una taxonomía "
    "cerrada de temas, y una taxonomía cerrada de secciones del informe.\n\n"
    ...
    "- RESOLUCIÓN DE FECHAS RELATIVAS Y MESES SIN AÑO: si la query menciona un "
    "mes sin año (ej. 'abril', 'en mayo', 'el mes de marzo'), asumí el año "
    "más reciente del rango del corpus que contenga ese mes. Si menciona "
    "'este año'/'el año pasado'/'el mes pasado', resolvélo contra el "
    "extremo más reciente del corpus (no contra el 2024 ni 2025 por default). "
    "Las fechas que emitas DEBEN caer dentro del rango del corpus — si no, "
    "devolvé null en vez de inventar.\n"
